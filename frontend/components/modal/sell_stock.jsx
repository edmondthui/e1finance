...
class SellStock extends React.Component {
  ...
  deleteHolding(e) {
    ...
    if (this.state.stock_id === null) {
      ...
      let holding = this.props.holdings.filter(
        (holding) => holding.stock_id === parseInt(this.state.stock_id)
      )[0];
      let activity = {
        activity: holding.stock_name,
        name: "Sell",
        value: this.state.value
          ? this.state.value
          : this.props.holdings[0].value,
  ...
  render() {
    let options = this.props.holdings.map((holding, idx) => (
      <option value={holding.stock_id} key={idx}>
        {holding.stock_name}
      </option>
    ));
    if (this.state.stock_id === null) {
        ...
        <div className="buy-stock-form-container">
          <form onSubmit={this.handleSubmit} className="buy-form">
                ...
                <h3>
                  {this.state.stock_id
                    ? `Sell ${
                        this.props.holdings.filter(
                          (holding) =>
                            holding.stock_id === parseInt(this.state.stock_id)
                        )[0].stock_name
                      }`
                    : "Sell Stock"}
                </h3>
            ...
            <div className="modal-submit-buttons">
              ...
              />
              <button className={"delete-holding"} onClick={this.deleteHolding}>
                Sell All Shares
              </button>
            </div>
          </form>
