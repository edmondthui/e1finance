...
export const fetchStockNews = (ticker) => {
  return (dispatch) => {
    ExternalAPIUtil.fetchCompanyNews(ticker).then((news) =>
      dispatch(receiveStockNews(news))
    );
  };
};
...
export const fetchAllNews = () => {
  return (dispatch) => {
    ExternalAPIUtil.fetchStockNews().then((news) =>
      dispatch(receiveAllNews(news))
    );
  };
};
...
export const fetchCompanyInfo = (ticker) => {
  return (dispatch) => {
    ExternalAPIUtil.fetchCompanyInfo(ticker).then((info) =>
